...
# some basic stack operations
	...
	} elsif ($cmd eq q(pop)) {	#pop
		pop @{$stack};
	} elsif ($cmd eq q(dup)) {	#dup
		$temp = pop @{$stack};
		push @{$stack}, $temp;
		push @{$stack}, $temp; 
	} elsif ($cmd eq q(exch)) {	#exch
		$temp = pop @{$stack};
		$tempb = pop @{$stack};
		push @{$stack}, $temp;
		push @{$stack}, $tempb;
	} elsif ($cmd eq q(clear)) {
		$stack = [];
	} elsif ($cmd eq q(remember)) {
		push @{$stack}, 'mk';
...
# math operations
		...
		$a = pop @{$stack};
		push @{$stack}, int($a);
	} elsif ($cmd eq q(smooth)) {
		$a = pop @{$stack};
		push @{$stack}, int($a + .5);
	} elsif ($cmd eq q(howmuch)) {
		$a = pop @{$stack};
		push @{$stack}, abs($a);
...
# SIHpoj jangwI'mey
		...
		$a = pop @{$stack};
		push @{$stack}, tan($a);
	} elsif ($cmd eq q(atan)) {	#equivalent to atan2() in C
		$d = pop @{$stack};
		$n = pop @{$stack};
...
# tlheghjangwI'
	...
	} elsif ($cmd eq q(strmeasure)) {
		$str = pop @{$stack};
		push @{$stack}, length($str);

	} elsif ($cmd eq q(strcut)) {
		$end = pop @{$stack};
		$begin = pop @{$stack};
		$str = pop @{$stack};
